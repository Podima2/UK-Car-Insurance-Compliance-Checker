...
export declare const PERSONAL_SIGN_PREFIX = "\u0019Constellation Signed Message:\n";
export declare const DATA_SIGN_PREFIX = "\u0019Constellation Signed Data:\n";
export declare class KeyStore {
    ...
    sign(privateKey: string, msg: string): Promise<string>;
    personalSign(privateKey: string, msg: string): Promise<string>;
    serialize(msg: string): string;
    dataSign(privateKey: string, msg: string): Promise<string>;
    verify(publicKey: string, msg: string, signature: string): any;
    verifyData(publicKey: string, msg: string, signature: string): any;
    ...
    generateTransactionWithHash(amount: number, toAddress: string, keyTrio: KeyTrio, lastRef: AddressLastRef, fee?: number): Promise<{
        hash: string;
        transaction: any;
    }>;
    generateTransaction(amount: number, toAddress: string, keyTrio: KeyTrio, lastRef: AddressLastRef, fee?: number): Promise<any>;
    generateTransactionWithHashV2(amount: number, toAddress: string, keyTrio: KeyTrio, lastRef: AddressLastRefV2, fee?: number): Promise<{
        hash: string;
        transaction: {
            value: {
                salt: string;
    ...
    brotliCompress(body: any): Promise<{
        normalized: any;
        compressed: any;
    }>;
    generateBrotliSignature(body: any, publicKey: string, privateKey: string): Promise<{
        value: any;
        proofs: {
            id: string;
            signature: string;
